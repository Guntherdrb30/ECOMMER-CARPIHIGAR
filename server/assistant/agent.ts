...
export async function* sendMessage(params: { text: string; customerId?: string }): AsyncGenerator<AgentChunk> {
  ...
  if (!q) {
    yield { type: 'text', message: '¿Podrías darme más detalles?' };
    return;
  }
  ...
  if (res?.success && Array.isArray(res.data) && res.data.length) {
    yield {
      type: 'text',
      message: 'Perfecto, ya lo tengo por aquí. Te muestro algunas opciones:',
    };
    ...
    yield {
      type: 'text',
      message: 'No encontré coincidencias exactas. ¿Me das alguna pista? (marca, tipo, color, medida)',
    };
  }
}
...
export async function* processIncomingAudio(_params: { audioBase64: string; customerId?: string }): AsyncGenerator<AgentChunk> {
  yield {
    type: 'text',
    message: 'Audio recibido. ¿Deseas que busque un producto específico?',
  };
}
