...
export async function confirmOrderAction(_prevState: any, formData: FormData) {
    ...
    try {
        ...
        if (!validMethods.includes(String(method))) {
            ...
            return { ok: false, error: 'M茅todo de pago inv谩lido' };
        }
        ...
        if (String(method) === 'PAGO_MOVIL' && !pmPhone.trim()) {
            ...
            return { ok: false, error: 'Para Pago M贸vil, el tel茅fono es obligatorio' };
        }
        if (String(method) === 'PAGO_MOVIL' && !pmPayerName.trim()) {
            ...
            return { ok: false, error: 'Para Pago M贸vil, el nombre del titular es obligatorio' };
        }
        if (String(method) === 'PAGO_MOVIL' && !pmBank.trim()) {
            ...
            return { ok: false, error: 'Para Pago M贸vil, el banco del titular es obligatorio' };
        }
        ...
        if (String(method) === 'TRANSFERENCIA') {
            if (String(paymentCurrency) === 'USD') {
                if (!depositPayerName.trim() || !depositPayerId.trim() || !depositBank.trim()) {
                    ...
                    return { ok: false, error: 'Para dep髎ito en USD, nombre, c閐ula y banco son obligatorios' };
                }
                ...
                if (!transferPayerName.trim() || !transferPayerId.trim()) {
                    ...
                    return { ok: false, error: 'Para transferencia en Bs, nombre y c閐ula son obligatorios' };
                }
            }
        }
        ...
        if (!selectedAddress) {
            ...
            return { ok: false, error: 'Debes agregar una direcci贸n de env铆o antes de confirmar.' };
        ...
        try {
            ...
            if (!digits || digits.length < 7) {
                ...
                return { ok: false, error: 'Tu tel閒ono es obligatorio en la direcci髇 de env韔.' };
            }
