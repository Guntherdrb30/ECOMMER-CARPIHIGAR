...
export async function createAllyProject(formData: FormData) {
  ...
  try {
    ...
    // Detect missing tables/columns (common during deploy without migrations)
    if (/relation .* does not exist|unknown table|does not exist|UndefinedTable/i.test(msg)) {
      return { ok: false, error: 'Error de base de datos: faltan tablas de proyectos/noticias. Ejecuta prisma migrate deploy en producción.' };
    }
