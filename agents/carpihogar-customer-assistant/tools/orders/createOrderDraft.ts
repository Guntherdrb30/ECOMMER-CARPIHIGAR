...
export async function createOrderDraft(input: { customerId: string }) {
  return transaction(async (db) => {
    // Active cart
    const c = await db.query('select id from carts where customer_id = $1 and status = $2 limit 1', [input.customerId, 'ACTIVE']);
    ...
    const ord = await db.query('insert into orders (customer_id, cart_id, total, status) values ($1,$2,$3,$4) returning id', [input.customerId, cartId, total, 'pending_confirmation']);
