...
export default function LeastSoldPanel() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const limit = Number(url.searchParams.get('low') || 10);
      ...
      const res = await fetch(`/api/inventory/least-sold?${qs.toString()}`);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-semibold">Menos vendidos (últimos {params.days} días)</h3>
        <div className="flex items-center gap-2 text-sm">
          <a className="border rounded px-2 py-0.5" href={`/dashboard/admin/inventario/print?tipo=low&${qsBase}`} target="_blank">Imprimir</a>
          <a className="border rounded px-2 py-0.5" href={`/api/reports/inventory?tipo=low&${qsBase}`} target="_blank">CSV</a>
          {rows.length > 0 && (
            <>
              ...
              <a className="border rounded px-2 py-0.5" href={`mailto:?subject=${encodeURIComponent('Menos vendidos')}&body=${encodeURIComponent(shareText)}`}>Email</a>
            </>
          )}
        </div>
      </div>
