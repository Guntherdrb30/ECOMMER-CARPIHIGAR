...
export function useAssistant() {
  ...
  useEffect(() => {
    ...
    booted.current = true;
    try {
      const saved = JSON.parse(localStorage.getItem('aiatlas_messages') || '[]');
  ...
  // Persist
  useEffect(() => {
    try { localStorage.setItem('aiatlas_messages', JSON.stringify(messages)); } catch {}
  }, [messages]);
  useEffect(() => {
    try { localStorage.setItem('aiatlas_open', open ? '1' : '0'); } catch {}
  }, [open]);
  ...
  const sendText = useCallback(async (text: string, opts?: { imageBase64?: string }) => {
    const userMsg: AssistantMessage = { id: uid(), role: 'user', type: 'text', content: text, ts: Date.now() };
    setMessages((prev) => [...prev, userMsg]);
    try {
      // Intento simple: iniciar flujo de compra si el usuario lo solicita
      if (/comprar ahora|proceder al pago|ir a pagar/i.test(text)) {
        try { await (continuePurchase as any)?.('start'); } catch {}
      } else if (/sí autorizo|si autorizo|\b\d{6}\b/i.test(text)) {
        const tokenMatch = text.match(/\b(\d{6})\b/);
        try { await (continuePurchase as any)?.('validateToken', { orderId: lastOrderId, confirmText: text, token: tokenMatch?.[1] }); } catch {}
      }
      const res = await fetch('/api/assistant/text', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, imageBase64: opts?.imageBase64 }) });
      if (res.body) {
        ...
        while (true) {
          ...
          while ((idx = buffer.indexOf("\n\n")) !== -1) {
            ...
            try {
              ...
              if (obj?.type === 'ui_control') { handleUiControl(obj); continue; }
              if (obj?.type === 'text') {
                setMessages((prev) => [...prev, { id: uid(), role: 'assistant', type: 'text', ts: Date.now(), content: String(obj?.message || obj?.content || '') }]);
              } else if (obj?.type === 'products') {
                setMessages((prev) => [...prev, { id: uid(), role: 'assistant', type: 'products', ts: Date.now(), data: obj?.products || obj?.data || [] }]);
              } else if (obj?.type === 'cart') {
                setMessages((prev) => [...prev, { id: uid(), role: 'assistant', type: 'cart', ts: Date.now(), data: obj?.data || {} }]);
              }
            } catch {}
          }
        }
      ...
      const arr: AssistantMessage[] = Array.isArray(json?.messages) ? json.messages : [];
  ...
  const continuePurchase = useCallback(async (step: PurchaseStep, input?: any) => {
    try {
      ...
      const json = await res.json();
      if (json?.message) setMessages((prev) => [...prev, { id: uid(), role: 'assistant', type: 'text', ts: Date.now(), content: json.message }]);
      if (json?.ui?.type === 'ui_control') {
        const a = String(json.ui.action || '').toLowerCase();
        if (a === 'open_cart') setView('cart');
        if (a === 'open_address_form') setView('address');
        if (a === 'show_products') setView('products');
        if (a === 'show_shipping') setView('products');
        if (a === 'show_order_summary') setView('products');
