import { getSales, getCommissions, getSellers, markCommissionPaid, markSaleReviewed } from \"@/server/actions/sales\";
      ...
      {allyPendingCount > 0 && (
        <div className=\"border border-amber-200 bg-amber-50 text-amber-800 px-3 py-2 rounded\">
          ...
          {allyPendingCount === 1 ? \"\" : \"s\"} de aliados por verificar.{\" "}\n          <a
            href=\"/dashboard/admin/ventas/aliados\"
            className=\"underline\"
          >
            Revisar ahora
          </a>
            ...
            <table className=\"w-full table-fixed text-sm\">
              ...
              <tbody>
                {orders.map((o: any) => {
                  ...
                  const originLabel = origin === \"ERP\" ? \"ERP\" : \"ONLINE\";
                  return (
                    <tr key={o.id} className=\"border-t\">
                      <td className=\"px-2 py-1 align-top\">
                        {new Date(o.createdAt).toLocaleString()}
                        <div className=\"text-xs text-gray-500\">{originLabel}</div>
                      </td>
                      <td className=\"px-2 py-1 align-top\">
                      ...
                      <td className=\"px-2 py-1 align-top\">
                        <div>{o.status}</div>
                        {o.saleType && (
                          <div className=\"text-xs text-gray-500\">{o.saleType}</div>
                        )}
                      </td>
                      <td className=\"px-2 py-1 align-top text-sm\">
                        {o.payment ? (
                          <div className=\"space-y-0.5\">
                            ...
                            {o.payment.reference && (
                              <div>Ref: {o.payment.reference}</div>
                            )}
                            {o.payment.method === \"PAGO_MOVIL\" && (
                              <div className=\"text-gray-600\">
                                {o.payment.payerName && (
                                  <div>Titular: {o.payment.payerName}</div>
                                )}
                                {o.payment.payerPhone && (
                                  <div>Tel: {o.payment.payerPhone}</div>
                                )}
                                {o.payment.payerBank && (
                                  <div>Banco: {o.payment.payerBank}</div>
                                )}
                              </div>
                            )}
                          </div>
                        ...
                        {o.status === \"PAGADO\" || o.status === \"COMPLETADO\" ? (
                          <div className=\"flex flex-wrap gap-2\">
                            <a
                              ...
                              href={/dashboard/admin/ventas//print?tipo=recibo}
                            >
                              Recibo (Bs)
                            </a>
                            <a
                              className=\"text-blue-600 hover:underline\"
                              href={/dashboard/admin/ventas//print?tipo=factura}
                            >
                              Factura (Bs)
                            </a>
                          </div>
