  ...
  return (
    <div className="container mx-auto p-4 space-y-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <h2 className="text-lg font-semibold mb-2">Tasa oficial BCV (Bs/USD)</h2>
        <p className="text-sm text-gray-600 mb-3">
          Esta tasa se usa para convertir las ventas legales a bolívares (facturas y recibos).
          Se obtiene automáticamente desde la página oficial del BCV.
        </p>
            ...
            action={async () => {
              ...
              try {
                ...
                const msg = encodeURIComponent(
                  String(e?.message || 'No se pudo actualizar la tasa'),
                );
                redirect('/dashboard/admin/ajustes/sistema?error=' + msg);
              }
              redirect(
                '/dashboard/admin/ajustes/sistema?message=Tasa%20BCV%20actualizada',
              );
            }}
          >
            <PendingButton
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Esta clave se usa para autorizar eliminaciones sensibles (abonos, productos,
          categorías, etc.).
        </p>
        <div className="mb-3 text-sm text-gray-700">
          Estado: {current ? 'Configurada' : 'No configurada'}
        </div>
        <form
          action={async (formData) => {
            ...
            try {
              ...
              redirect(
                '/dashboard/admin/ajustes/sistema?message=Clave%20cambiada',
              );
            } catch (e: any) {
              const m = encodeURIComponent(
                String(e?.message || 'No se pudieron guardar los cambios'),
              );
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Configura cómo se valida que el usuario root reciba códigos de recuperación
          por WhatsApp.
        </p>
        <form
          action={async (formData) => {
            ...
            try {
              ...
              redirect(
                '/dashboard/admin/ajustes/sistema?message=Root%20actualizado',
              );
            } catch {
              redirect(
                '/dashboard/admin/ajustes/sistema?error=No%20se%20pudo%20guardar',
              );
            }
          }}
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Estos datos se usan en el checkout y comprobantes para mostrar al cliente
          cómo pagar (Zelle, Pago Móvil, Banesco, Mercantil).
        </p>
        <form
          action={async (formData) => {
            'use server';
            try {
              ...
              redirect(
                '/dashboard/admin/ajustes/sistema?message=Pagos%20actualizados',
              );
            } catch {
              redirect(
                '/dashboard/admin/ajustes/sistema?error=No%20se%20pudo%20guardar',
        ...
        >
          ...
          <div className="md:col-span-3 font-semibold mt-4">
            VES · Pago Móvil
          </div>
          <div>
          ...
          <div className="md:col-span-3 font-semibold mt-4">
            Depósitos/Transferencias · Banesco
          </div>
          <div>
            <label className="block text-sm text-gray-700">
              Nombre del titular
          ...
          <div className="md:col-span-3 font-semibold mt-4">
            Depósitos/Transferencias · Mercantil
          </div>
          <div>
            <label className="block text-sm text-gray-700">
              Nombre del titular
          ...
          <div>
            ...
            <input
              ...
              defaultValue={
                (siteSettings as any).paymentMercantilAccount || ''
              }
              className="border rounded px-2 py-1 w-full"
              placeholder="0000-0000-00-0000000000"
            />
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <h2 className="text-lg font-semibold mb-2">
          Ganancias de productos (márgenes %)
        </h2>
        <p className="text-sm text-gray-600 mb-3">
          Se usan para calcular precios a partir del costo al importar compras o
          productos por CSV.
        </p>
        <form
          action={async (formData) => {
        ...
        >
          <div>
            ...
            <input
              ...
              min={0}
              defaultValue={Number(
                (siteSettings as any).defaultMarginClientPct ?? 40,
              )}
              className="border rounded px-2 py-1 w-full"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={0}
              defaultValue={Number(
                (siteSettings as any).defaultMarginAllyPct ?? 30,
              )}
              className="border rounded px-2 py-1 w-full"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={0}
              defaultValue={Number(
                (siteSettings as any).defaultMarginWholesalePct ?? 20,
              )}
              className="border rounded px-2 py-1 w-full"
            />
          </div>
