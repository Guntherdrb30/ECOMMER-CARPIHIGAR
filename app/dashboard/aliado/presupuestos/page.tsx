...
export default async function AliadoPresupuestosPage({ searchParams }: { searchParams?: Promise<{ q?: string; estado?: string; desde?: string; hasta?: string }> }) {
  ...
  const quotes = await getMyAllyQuotes({ q: q || undefined, status: estado || undefined, from: desde || undefined, to: hasta || undefined });
  return (
    <div className="container mx-auto p-4 space-y-4">
      ...
      <form method="get" className="bg-white p-4 rounded-lg shadow grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
        ...
        <div className="form-actions">
          ...
          <a className="px-3 py-1 rounded border" href="/dashboard/aliado/presupuestos">Limpiar</a>
        </div>
      </form>
              ...
              {quotes.map((q: any) => (
                <tr key={q.id}>
                  ...
                  <td className="border px-3 py-2 text-right space-x-2">
                    <a className="text-blue-600 hover:underline" href={`/dashboard/aliado/presupuestos/${q.id}`}>Ver</a>
