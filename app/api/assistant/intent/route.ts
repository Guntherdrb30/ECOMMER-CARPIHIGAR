...
{
  "intent": "add_to_cart"|"remove_from_cart"|"buy"|"set_address"|"set_payment"|"confirm",
  "entities": { "productName"?: string, "quantity"?: number, "addressId"?: string, "paymentMethod"?: string, "color"?: string, "model"?: string }
}
Si no es claro, usa intent "buy" y entities vacíos.`;
      ...
      if (res.ok) {
        ...
        try { const parsed = JSON.parse(content); return NextResponse.json(parsed); } catch {}
      }
    }
    const low = text.toLowerCase();
    let intent: any = 'buy';
    if (/agrega|añade|meter|pon|agregar|añadir/.test(low)) intent = 'add_to_cart';
    if (/quitar|remueve|elimina|saca/.test(low)) intent = 'remove_from_cart';
    if (/comprar|cómpralo|lo quiero|quiero comprar/.test(low)) intent = 'buy';
    if (/direcci[óo]n|env[ií]alo a|enviar a/.test(low)) intent = 'set_address';
    if (/pago|pagar|m[ée]todo de pago|zelle|transferencia|pago m[óo]vil/.test(low)) intent = 'set_payment';
    ...
    const prod = low.replace(/(comprar|agrega|añade|meter|pon|quiero|deseo)/g, '').trim();
