...
async function main() {
  const email = (process.env.ROOT_EMAIL || 'root@carpihogar.com').toLowerCase();
  const plain = process.env.ROOT_PASSWORD || '';

  if (!plain) {
    console.error('Missing ROOT_PASSWORD env var. Example:');
    console.error('  ROOT_EMAIL=root@carpihogar.com ROOT_PASSWORD=StrongPass123! npm run root:create');
    process.exit(1);
  }
